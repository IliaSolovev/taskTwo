        ...
        if (daysOfTheCurrentMonth[0][i] > 7) {
            ...
            displayDays = [...displayDays,
                <DayBlock key={`${daysOfTheCurrentMonth[0][i]}-${currentMonth}-${currentYear}`}
                          ...
                          isDayBetweenActive={isDayBetweenActive(activeDays, daysOfTheCurrentMonth[0][i])}
                          listener={listener}/>]
        }
    }
    for (let i = 1; i < 4; i++) {
        daysOfTheCurrentMonth[i].map((day, id) => {
            displayDays = [...displayDays, <DayBlock key={`${id}-${day}-${currentMonth}-${currentYear}`}
                                                     ...
                                                     isDayBetweenActive={isDayBetweenActive(activeDays, day)}
                                                     listener={listener}/>];
        })
    }
    for (let i = 4; i < 6; i++) {
...
const isDayBetweenActive = (activeDays, day) => {
    return day > activeDays[0] && day < activeDays[1]
};
