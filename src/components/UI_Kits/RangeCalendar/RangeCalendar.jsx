...
const RangeCalendar = ({classes, onSubmit, onClear, ...rest}) => {
    ...
    const onSelectDate = (date) => {
        if (activeDays[0] === null && activeDays[1] === null) {
            setActiveDay([date, date])
        } else if (date < activeDays[0]) {
            setActiveDay([date, activeDays[1]])
        } else {
            setActiveDay([activeDays[0], date])
        }
    };
    const submit = () => {
        if (!activeDays) {
